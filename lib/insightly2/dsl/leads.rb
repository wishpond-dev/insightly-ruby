...
module Insightly2
  module DSL::Leads
    ...
    # @param [Boolean] include_converted by default converted leads are not included,
    #        pass in "true" to include them in the response (optional).
    # @return [Array, nil].
    def get_leads(ids: [], email: '', tag: '')
      url = Utils::UrlHelper.build_url(path: "Leads", params: {ids: ids.join(','),
        email: email, tag: tag})
